...
def eval_paradigm_accuracy_random(results_frame):
    ...
    eval_frame = results_frame.drop_duplicates(subset=['target_msd', 'paradigm_i'], keep='first') # NOTE: this assumes the data was properly shuffled before
    total = 0
    num_correct = 0
    correct_paradigms = []
    for paradigm_i in set(eval_frame['paradigm_i'].values):
        paradigm = eval_frame.loc[eval_frame['paradigm_i'] == paradigm_i]
        if (paradigm['target'] == paradigm['predictions']).all():
            num_correct += 1
...
def eval_paradigm_accuracy_max(results_frame):
    ...
    eval_frame = results_frame.drop_duplicates(subset=['target', 'paradigm_i'], keep='first') 
    total = 0
    num_correct = 0
    correct_paradigms = []
    for paradigm_i in set(eval_frame['paradigm_i'].values):
...
def eval_accuracy_oracle(results_frame): 
    ...
    results_frame['bleu'] = results_frame[['predictions', 'target']].apply(lambda row: distance(row.predictions, row.target), axis=1)
    results_frame = results_frame.sort_values(by='bleu', ascending=True)
    eval_frame = results_frame.drop_duplicates(subset=['target_msd', 'paradigm_i'], keep='first') 
    total = 0
    num_correct = 0
    correct_paradigms = []
    for paradigm_i in set(eval_frame['paradigm_i'].values):
...
def eval_accuracy_per_row(results_frame):
    ...
    correct = sum(results_frame['predictions'] == results_frame['target'])
