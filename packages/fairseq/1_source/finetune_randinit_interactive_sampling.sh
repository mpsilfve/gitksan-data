...
# fairseq-generate --path $ONE_SOURCE_CHECKPT_PREFIX/checkpoint_best.pt --sampling --beam 20 --nbest 20  --source-lang src \
# --target-lang tgt $ONE_SOURCE_PREPROC_SAVE_DIR < $ONE_SOURCE_DEVSRC > $ONE_SOURCE_SAVEPREF/results_dev_sampling.txt

# fairseq-generate --path $ONE_SOURCE_CHECKPT_PREFIX/checkpoint_best.pt --sampling --beam 20 --nbest 20 --source-lang src \
# --target-lang tgt $ONE_SOURCE_PREPROC_SAVE_DIR < $ONE_SOURCE_SEEN_TEST > $ONE_SOURCE_SAVEPREF/results_seen_test_sampling.txt

# fairseq-generate $ONE_SOURCE_PREPROC_SAVE_DIR --path $ONE_SOURCE_CHECKPT_PREFIX/checkpoint_best.pt --sampling --beam 20 --nbest 20 --source-lang src \
# --target-lang tgt  > $ONE_SOURCE_SAVEPREF/results_sampling.txt

fairseq-interactive --path $ONE_SOURCE_CHECKPT_PREFIX/checkpoint_best.pt --sampling --beam 20 --nbest 20 --source-lang src \
--target-lang tgt $ONE_SOURCE_PREPROC_SAVE_DIR < $ONE_SOURCE_DEVSRC > $ONE_SOURCE_SAVEPREF/results_dev_sampling.txt
...
fairseq-interactive --path $ONE_SOURCE_CHECKPT_PREFIX/checkpoint_best.pt --sampling --beam 20 --nbest 20 --source-lang src \
--target-lang tgt $ONE_SOURCE_PREPROC_SAVE_DIR < $ONE_SOURCE_UNSEEN_TEST > $ONE_SOURCE_SAVEPREF/results_unseen_test_sampling.txt
